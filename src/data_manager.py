...
def validar_datos(df):
    ...
    Lanza un ValueError si los datos no son válidos.
    """
...
def preparar_datos(df):
    ...
    # Mapeo para codificación de variables categóricas
    map_horario = {'Mañana': 0, 'Tarde': 1, 'Noche': 2}
    map_si_no = {'No': 0, 'Sí': 1}
    map_frecuencia = {'Nunca': 0, 'Rara vez': 1, 'A veces': 2, 'Frecuentemente': 3, 'Siempre': 4}

    df_encoded['Horario_preferido'] = df_encoded['Horario_preferido'].map(map_horario)
    df_encoded['Tiene_espacio_estudio'] = df_encoded['Tiene_espacio_estudio'].map(map_si_no)
    df_encoded['Frecuencia_repaso_apuntes'] = df_encoded['Frecuencia_repaso_apuntes'].map(map_frecuencia)
    df_encoded['Usa_tecnicas_estudio'] = df_encoded['Usa_tecnicas_estudio'].map(map_si_no)

    # Lista de características para escalar y aplicar PCA
    features = [
    ...
    # Análisis de Componentes Principales (PCA) para visualización 2D
